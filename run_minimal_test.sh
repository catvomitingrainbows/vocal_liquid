...
echo "=== Running Minimal Icon Test ==="
echo "This is the simplest possible test of status bar icon coloring"

# Ensure old versions aren't running
pkill -f IconTest 2>/dev/null || true

# Create a super minimal test file
cat > super_minimal.swift << 'EOF'
import Cocoa
...
# Compile
echo "Compiling super minimal test app..."
swiftc -o IconTest super_minimal.swift

# Make executable
chmod +x IconTest

# Run
echo "Running test... watch the menu bar"
echo "App will automatically turn icon red, then try to reset it"
echo "If icon stays red, use the 'Replace Status Item' menu option"
./IconTest